...
@Component({
  selector: 'app-infected-by-age',
  templateUrl: './infected-by-age.component.html',
  styleUrls: ['./infected-by-age.component.css']
})
export class InfectedByAgeComponent implements OnInit {
  @Input() city: string;
...
extractData(data){
  ...
  let data_grouped = d3.nest<unknown, number>().key(d => d['EVENT']).key(function(d) :string{
    ...
    if (0 < d['INFECTED_AGE'] && d['INFECTED_AGE'] <= 10) {
      age_group = '01-10';
    } else if (10 < d['INFECTED_AGE'] && d['INFECTED_AGE'] <= 20)  {
      age_group = '11-20';
    } else if (20 < d['INFECTED_AGE'] && d['INFECTED_AGE'] <= 30)  {
      age_group = '21-30';
    } else if (30 < d['INFECTED_AGE'] && d['INFECTED_AGE'] <= 40)  {
      age_group = '31-40';
    } else if (40 < d['INFECTED_AGE'] && d['INFECTED_AGE'] <= 50)  {
      age_group = '41-50';
    } else if (50 < d['INFECTED_AGE'] && d['INFECTED_AGE'] <= 60)  {
      age_group = '51-60';
    } else if (60 < d['INFECTED_AGE'] && d['INFECTED_AGE'] <= 70)  {
      age_group = '61-70';
    } else if (70 < d['INFECTED_AGE'] && d['INFECTED_AGE'] <= 80)  {
      age_group = '71-80';
    } else if (80 < d['INFECTED_AGE'] && d['INFECTED_AGE'] <= 90)  {
      age_group = '81-90';
    } else if (90 < d['INFECTED_AGE'] && d['INFECTED_AGE'] <= 100) {
      age_group = '91-100';
    }  
    return age_group;
  })
...
createGraph(data, population) {
  ...
  this.chart = am4core.create("chartdiv_byage_"+this.city, am4charts.XYChart);
